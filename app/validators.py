...
def validate_carrier_remittance_csv(df: pd.DataFrame) -> List[str]:
    ...
    required_columns = {'policy_id', 'agent_id', 'carrier', 'paid_date', 'amount', 'status'}
    missing_columns = required_columns - set(df.columns)
    if missing_columns:
        errors.append(f"Missing required columns in carrier remittance: {missing_columns}")
    ...
    for idx, row in df.iterrows():
        ...
        # Validate policy_id
        if pd.isna(row['policy_id']) or str(row['policy_id']).strip() == '':
        ...
        try:
            if pd.isna(row['paid_date']):
                ...
                datetime.strptime(str(row['paid_date']), '%Y-%m-%d')
        except ValueError:
            row_errors.append(f"paid_date must be in YYYY-MM-DD format, got: {row['paid_date']}")
        ...
        try:
            ...
            row_errors.append(f"amount must be a valid number, got: {row['amount']}")

        # Validate status
        valid_statuses = {'active', 'cancelled'}
        if pd.isna(row['status']) or str(row['status']).lower() not in valid_statuses:
            row_errors.append(f"status must be one of {valid_statuses}, got: {row['status']}")

        if row_errors:
            errors.append(f"Row {idx + 2}: {'; '.join(row_errors)}")  # +2 for header and 0-based index
...
def validate_crm_policies_csv(df: pd.DataFrame) -> List[str]:
    ...
    for idx, row in df.iterrows():
        ...
        try:
            if pd.isna(row['submit_date']):
                row_errors.append("submit_date cannot be empty")
            else:
                datetime.strptime(str(row['submit_date']), '%Y-%m-%d')
        except ValueError:
            row_errors.append(f"submit_date must be in YYYY-MM-DD format, got: {row['submit_date']}")

        # Validate ltv_expected
        try:
            ltv = float(row['ltv_expected'])
            if ltv < 0:
                row_errors.append(f"ltv_expected must be non-negative, got: {ltv}")
        except (ValueError, TypeError):
            row_errors.append(f"ltv_expected must be a valid positive number, got: {row['ltv_expected']}")
...
def clean_and_prepare_data(carrier_df: pd.DataFrame, crm_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    logger.info(f"Cleaned data: {len(carrier_clean)} carrier records, {len(crm_clean)} CRM records")
