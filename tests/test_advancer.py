...
def post_advance_quote(carrier, crm):
    files = {"carrier_remittance": ("carrier.csv", carrier, "text/csv"), "crm_policies": ("crm.csv", crm, "text/csv")}
...
class TestAdvanceQuoteSuccess:
    ...
    def test_case_insensitive_extension(self, valid_carrier_csv, valid_crm_csv):
        ...
        files = {"carrier_remittance": ("carrier.CSV", valid_carrier_csv, "text/csv"), "crm_policies": ("crm.Csv", valid_crm_csv, "text/csv")}
...
class TestAdvanceQuoteValidation:
    def test_invalid_file_type(self):
        ...
        files = {"carrier_remittance": ("carrier.txt", "some content", "text/plain"), "crm_policies": ("crm.txt", "some content", "text/plain")}
        response = client.post("/v1/advance-quote", files=files)
    ...
    def test_missing_filename(self):
        ...
        files = {"carrier_remittance": ("", "some content", "text/csv"), "crm_policies": ("", "some content", "text/csv")}
        response = client.post("/v1/advance-quote", files=files)
        assert response.status_code == 422

    def test_empty_csv(self):
        """Test with empty CSV files."""
        files = {"carrier_remittance": ("carrier.csv", "", "text/csv"), "crm_policies": ("crm.csv", "", "text/csv")}
        response = client.post("/v1/advance-quote", files=files)
        assert response.status_code == 400
    ...
    def test_encoding_error(self, valid_crm_csv):
        ...
        files = {"carrier_remittance": ("carrier.csv", invalid_utf8, "text/csv"), "crm_policies": ("crm.csv", valid_crm_csv, "text/csv")}
        response = client.post("/v1/advance-quote", files=files)
        assert response.status_code == 400
    ...
    def test_mixed_file_types(self, valid_carrier_csv):
        ...
        files = {"carrier_remittance": ("carrier.csv", valid_carrier_csv, "text/csv"), "crm_policies": ("crm.txt", "some text content", "text/plain")}
        response = client.post("/v1/advance-quote", files=files)
        assert response.status_code == 400
        assert "CSV files" in response.json()["detail"]
    ...
    def test_invalid_csv_format(self, valid_crm_csv):
        ...
        files = {"carrier_remittance": ("carrier.csv", carrier_csv, "text/csv"), "crm_policies": ("crm.csv", valid_crm_csv, "text/csv")}
        response = client.post("/v1/advance-quote", files=files)
...
def test_compute_quotes_basic():
    ...
    crm = pd.DataFrame({"policy_id": ["P001"], "agent_id": ["A1"], "submit_date": ["2025-06-15"], "ltv_expected": [800]})
...
def test_compute_quotes_submit_date_within_7_days_not_eligible():
    ...
    carrier = pd.DataFrame({"policy_id": ["P001"], "agent_id": ["A1"], "paid_date": ["2025-07-01"], "amount": [200], "status": ["active"]})
    crm = pd.DataFrame(
        {
            "policy_id": ["P001"],
            "agent_id": ["A1"],
...
def test_compute_quotes_cancellation_example():
    ...
    crm = pd.DataFrame({"policy_id": ["PCLAW1"], "agent_id": ["A005"], "submit_date": ["2025-06-10"], "ltv_expected": [1000]})
    result = compute_quotes(carrier, crm)
